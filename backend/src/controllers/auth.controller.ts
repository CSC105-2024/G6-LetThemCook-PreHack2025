...
import * as authModel from '../modles/user.model.ts';
...
export const profileUpdate = async(c:Context)=>{
    ...
    try{
        const updatedUser = await authModel.updateUserProfile(userId, username,bio ,pfpURL )
        return c.json({
            success:true,
            msg:"Updated profile",
            data: updatedUser
    ...
    console.error(e);
    return c.json({ success: false, msg: "Failed to update user profile." }, 500);
  }
}